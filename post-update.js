...
function postToSlack(appInfo, submissionStartDate) {
  ...
  var message = `The status of your app *${
    appInfo.name
  }* has been changed to *${appInfo.status}*`;
  var attachment = slackAttachment(appInfo, submissionStartDate);
  var params = {
    channel: process.env.CHANNEL_NAME || "#ios-app-updates",
    text: message,
  ...
  client.chat
    .postMessage(params)
    .then(() => { console.log(`Post status to slack successfully for "${appInfo.name}"`) })
    .catch(console.error);
}

function slackAttachment(appInfo, submissionStartDate) {
  var attachment = {
    fallback: `The status of your app ${appInfo.name} has been changed to ${
      appInfo.status
    }`,
    ...
    title_link: `https://itunesconnect.apple.com/WebObjects/iTunesConnect.woa/ra/ng/app/${
      appInfo.appId
    }`,
    fields: [
      {
    ...
    footer_icon:
      "https://devimages.apple.com.edgekey.net/app-store/marketing/guidelines/images/app-store-icon.png",
  ...
  if (
    ...
    var elapsedTime = moment(submissionStartDate).fromNow(true);
